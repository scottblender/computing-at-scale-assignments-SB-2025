...
int test_gauss_legendre(std::vector<double> coeffs, double expected_result) {
    ...
    double result = polynomial_integrate(1, 0, coeffs_, 0.0, 1.0, 3);  // Integrating x^2 from 0 to 1
...
int test_gauss_lobatto(std::vector<double> coeffs, double expected_result) {
    std::vector<double> coeffs_ = coeffs;  // Polynomial x^2
    double result = polynomial_integrate(2, 0, coeffs_, 0.0, 1.0, 3);  // Integrating x^2 from 0 to 1
    double expected_result_ = expected_result;  // Integral of x^2 from 0 to 1 is 1/3
    if (std::abs(result - expected_result_) < 1e-6) {
...
int test_gauss_chebyshev_case1(std::vector<double> coeffs, double expected_result) {
    std::vector<double> coeffs_ = coeffs;  // Polynomial x^2
    double result = polynomial_integrate(3, 1, coeffs_, 0.0, 1.0, 3);  // Integrating x^2/sqrt(1-x^2) from 0 to 1 using Gauss-Chebyshev case 1
...
int test_gauss_chebyshev_case2(std::vector<double> coeffs, double expected_result) {
    std::vector<double> coeffs_ = coeffs;  // Polynomial x^2
    double result = polynomial_integrate(3, 2, coeffs_, 0.0, 1.0, 3);  // Integrating x^2*sqrt(1-x^2) from 0 to 1 using Gauss-Chebyshev case 1
...
int main() {
    int total_tests = 0;
    // x^2
    total_tests += test_gauss_legendre({1, 0, 0},1.0/3.0);
    total_tests += test_gauss_lobatto({1, 0, 0},1.0/3.0);
    total_tests += test_gauss_chebyshev_case1({1, 0, 0},M_PI/4.0);
    total_tests += test_gauss_chebyshev_case2({1, 0, 0},M_PI/16.0);
    
    // x
    total_tests += test_gauss_legendre({0, 1, 0},1.0/2.0);
    total_tests += test_gauss_lobatto({0, 1, 0},1.0/2.0);
    total_tests += test_gauss_chebyshev_case1({0, 1, 0},1.0);
    total_tests += test_gauss_chebyshev_case2({0, 1, 0},1.0/3.0);
